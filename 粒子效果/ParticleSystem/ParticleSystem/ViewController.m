...
- (void)viewDidLoad {
    ...
    UISegmentedControl *seg = [[UISegmentedControl alloc] initWithFrame:CGRectMake(100, 100, 200, 50)];
    seg.backgroundColor = [UIColor yellowColor];
    [seg insertSegmentWithTitle:@"1" atIndex:0 animated:YES];
    [seg insertSegmentWithTitle:@"2" atIndex:1 animated:YES];
    [seg insertSegmentWithTitle:@"3" atIndex:2 animated:YES];
    [seg insertSegmentWithTitle:@"4" atIndex:3 animated:YES];
    [seg setSelectedSegmentIndex:0];
    [seg addTarget:self action:@selector(changeIndex:) forControlEvents:UIControlEventValueChanged];
    ...
    void(^blockA)() = ^{
        ...
        //重力
        self.particleEffect.gravity = DefaultGravity;
...
- (void)changeIndex:(UISegmentedControl *)seg {
    //选择不同的效果
    self.currentEmitterIndex = [seg selectedSegmentIndex];
}
